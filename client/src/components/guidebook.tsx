...
const guideCardStyles = [
  ...
  { num: "GUIDE-06", roleColor: "var(--accent)", roleBg: "var(--accent-tint)", roleBorder: "var(--accent-tint2)", linkUrls: ["https://kemenparekraf.go.id"] },
];
    ...
    <div>
      <div className="relative z-[5] max-w-5xl mx-auto pt-10 pb-8 px-6 md:px-10">
        ...
        <h1 className="font-heading font-black text-[50px] leading-[1.06] tracking-[-1.5px] mb-[18px] max-md:text-[34px]" style={{ color: "var(--txt)" }}>
          ...
          <span className="text-[var(--accent)]">{t.guide.heroTitle2}</span>
        </h1>
      ...
      <div className="relative z-[5] max-w-5xl mx-auto px-6 md:px-10 pb-16">
        <div className="grid grid-cols-2 gap-[14px] mb-12 max-md:grid-cols-1">
          {translatedGuideCards.map((card) => (
            ...
            >
              <div className="font-heading text-[9px] font-bold tracking-[2.5px] uppercase text-[var(--accent2)] mb-[10px]">
                {card.num}
              </div>
              <div className="font-heading font-extrabold text-[15px] mb-2 tracking-[-0.2px]" style={{ color: "var(--txt)" }}>
                {card.title}
              </div>
              <span
              ...
              <div className="text-[13px] font-light leading-[1.65] mb-4" style={{ color: "var(--t2)" }}>{card.desc}</div>
              <div className="flex flex-wrap gap-[6px]">
                {card.links.map((link) => (
                  <a
                    ...
                    className="inline-flex items-center gap-[5px] text-[11px] font-bold py-[5px] px-[10px] rounded no-underline text-[var(--accent)] bg-[var(--accent-tint)] border border-[var(--accent-tint2)] hover-elevate"
                  >
                    {"\u2197"} {link.label}
                  </a>
                ))}
        ...
        <div className="mb-12">
          <h3 className="font-heading font-extrabold text-[16px] text-[var(--accent)] mb-6 pb-[10px] border-b tracking-[-0.2px]" style={{ borderColor: "var(--accent-tint)" }}>
            {t.guide.timelineTitle}
          </h3>
          <div className="relative pl-8">
            <div className="absolute left-[8px] top-[8px] bottom-[8px] w-[1px] bg-gradient-to-b from-[var(--accent2)] to-[var(--accent-tint)]" />
            {translatedTimeline.map((item, i) => (
              <div key={i} className="relative mb-[26px]">
                <div className="absolute left-[-28px] top-[5px] w-[11px] h-[11px] rounded-full border-[1.5px] border-[var(--accent2)]" style={{ background: "var(--app-node-bg)" }} />
                <div className="font-heading text-[9px] font-bold tracking-[2px] uppercase text-[var(--accent2)] mb-1">
                  {item.week}
                </div>
                <div className="font-heading font-bold text-[14px] mb-[5px] tracking-[-0.1px]" style={{ color: "var(--txt)" }}>
                  {item.title}
                </div>
                <div className="text-[13px] font-light leading-[1.6]" style={{ color: "var(--t3)" }}>{item.desc}</div>
              </div>
            ))}
          </div>
        </div>

        <div className="mt-4 pl-5 border-l-2 border-l-[rgba(100,116,139,0.25)] text-[12px] font-light leading-[1.7] italic" style={{ color: "var(--t3)" }}>
          {t.guide.disclaimer}
        </div>
      </div>
    </div>
